...
def proc_logging(id, info, train_info):
    info = "[IBN_ResNet18 Result for test_session="+str(id)+"] "+info
    path = "./log/IBN_ResNet18/"+train_info+"/"
...
def global_logging(info, train_info):
    info = "[IBN_ResNet18 Final Result] "+info
    path = "./log/IBN_ResNet18/"+train_info+"/"
...
def train(id, bs=256, ne=10, lr=0.0001, train_info=""):
    ...
    model = IBN_ResNet18(num_classes=4).to(device)
    ...
    for epoch in range(n_epoch):
        ...
        '''save model'''
        save_path = "./model/IBN_ResNet18/"+train_info+"/"
...
if __name__=="__main__":
    ...
    parser.add_argument("--ne", type=int, default=20)
    parser.add_argument("--lr", type=float, default=0.0001)
    args = parser.parse_args()
    train_info = str(args.bs)+"_"+str(args.ne)+"_"+str(args.lr)
    os.system("rm -r ./log/IBN_ResNet18/"+train_info+"/")
    os.system("rm -r ./model/IBN_ResNet18/"+train_info+"/")
