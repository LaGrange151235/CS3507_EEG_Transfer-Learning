...
def test(model, test_dataloader, device):
    ...
    with torch.no_grad():
        for i in range(len(test_dataloader)):
            ...
            target_value, target_label, _ = target_data
            target_value = target_value.to(device)
            target_label = target_label.long().to(device)
            batch_size = len(target_label)

            class_output, _ = model(input_data=target_value)
...
def train(id):
    ...
    train_domain = np.load(dataset_path+"train_domain.npy")
    test_data = np.load(dataset_path+"test_data.npy")
    test_label = np.load(dataset_path+"test_label.npy")
    test_domain = np.load(dataset_path+"test_domain.npy")
    ...
    model = DG_DANN(momentum=momentum).to(device)
    ...
    train_dataset = npArraryDataset(train_data, train_label, train_domain)
    test_dataset = npArraryDataset(test_data, test_label, test_domain)
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            source_value, source_label, source_domain = source_data
            ...
            source_domain = source_domain.long().to(device)
            #source_domain_label = source_domain_label.long().to(device)

            source_class_pred, source_domain_pred = model(input_data=source_value, class_alpha=alpha, domain_alpha=alpha)
            ...
            source_domain_loss = domain_loss(source_domain_pred, source_domain)
            ...
            loss = source_class_loss + source_domain_loss
            loss.backward()
            optimizer.step()
...
#
    ...
    #print("[DANN Final Result] total_models: %d, train_acc: %.4f, test_acc: %.4f" % (len(test_acc_list), avg_train_acc, avg_test_acc))