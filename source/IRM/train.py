...
class IRM(nn.Module):
    ...
    def penalty(self, criterion, logits, y):
        scale = torch.ones((1, 4)).to(y.device).requires_grad_()
...
def train(id):
    ...
    model = IRM(penalty_weight).to(device)
    ...
    for epoch in range(n_epoch):
        ...
        for idx, train_data in enumerate(train_dataloader):
            ...
            penalty = model.penalty(criterion, pred, train_label)
            loss = loss + model.penalty_weight*penalty
            loss.backward()
            optimizer.step()
